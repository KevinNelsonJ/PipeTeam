//we need all these things for the code to run, 
...
namespace Step35
{
  ...
  //to make the code easier to edit, all of the parameters for the equations
  //are kept in a separate file.  This includes the formulation of the equation 
  // for the auxiliary variable $\phi$, for which we declare an <code>enum</code>.
  // the following is a class that will read that file.
  ...
  //So, everything above was handled in step 19, we'll have to go read that
...
//this is a weird vector it keeps track of our vector quantity over time and space
    template <int dim>
  ...
  create_triangulation_and_dofs (const unsigned int n_refines)
  {
    ...
    {
      std::string filename = "input/accordian.ucd";   	//change this line to our grid file name
      std::ifstream file (filename.c_str());
      Assert (file, ExcFileNotOpen (filename.c_str()));
      grid_in.read_ucd (file);			//check the ucd file format so we can get out
    }						//mesh done!!
    ...
    {
      // Output the mesh
      std::ofstream out("data/accordian.eps");
      GridOut grid_out;
  ...
  NavierStokesProjection<dim>::initialize()
  {
    ...
    vel_Laplace_plus_Mass.add (1./Re, vel_Laplace);	//the diffusive term
    vel_Laplace_plus_Mass.add (1.5/dt, vel_Mass);	//from the time dependent step

    EquationData::Pressure<dim> pres (t_0);
    VectorTools::interpolate (dof_handler_pressure, pres, pres_n_minus_1);
//the previous pressure into a discrete form
    pres.advance_time (dt);
    VectorTools::interpolate (dof_handler_pressure, pres, pres_n);
//put our current pressure into a discrete form
    phi_n = 0.;
    phi_n_minus_1 = 0.;
    for (unsigned int d=0; d<dim; ++d)
      {
      ...
      }//same thing as pressure above, velocity is a vector
  }
  ...
  {
    for (unsigned int d=0; d<dim; ++d)
      u_star[d].equ (2., u_n[d], -1, u_n_minus_1[d]);	//step 0 u*
  }
...
//the ultimate goal is to solve for the velocity at the next time step. to this,
//we will add terms to the other side of the equation
    for (unsigned int d=0; d<dim; ++d) //for each dimension
      {
        force[d] = 0.;
        v_tmp.equ (2./dt,u_n[d],-.5/dt,u_n_minus_1[d]);//these are the diffusive terms from step 1
        vel_Mass.vmult_add (force[d], v_tmp);		//multiply the force by the previous velocity and add them to the vel_Mass matrix

//quick question, what is vel_Mass??

        pres_Diff[d].vmult_add (force[d], pres_tmp);	
//multiply forcing function by temp pressure to get the differential pressure
        u_n_minus_1[d] = u_n[d];			//update the old values of velocity

        vel_it_matrix[d].copy_from (vel_Laplace_plus_Mass);
        vel_it_matrix[d].add (1., vel_Advection);	//add the advective u_k+1 terms
        ...
        MatrixTools::apply_boundary_values (boundary_values,
                                            ...
                                            u_n[d],
                                            force[d]);
//This line applies dirichlet conditions to the system matrix and two vectors in a certain direction
      }


    Threads::TaskGroup<void> tasks;		
//process thestuff in parallel, because it is faster
    for (unsigned int d=0; d<dim; ++d)
      {
        if (reinit_prec)
    ...
    tasks.join_all();//this here is our solver for each component of velocity
  }	//all of the components are solved in parallel and the threads are rejoined 


//the mystery solver, how does it work team?
  template <int dim>
  void
  ...
  {
    vel_Advection = 0.;
    AdvectionPerTaskData data (fe_velocity.dofs_per_cell);//defines the advection matrix
...
//I don't know what this does,  as far as I know, it builds the advection matrix and thats it. I think dat is the u^k and scratch is u*
...
int main()
{
  try
    {
      ...
      data.read_data ("input/parameter-file.prm");	//the specific property file

      deallog.depth_console (data.verbose ? 2 : 0);

      NavierStokesProjection<2> test (data);		//our problem is 3D, called test
  ...
  std::cout << "----------------------------------------------------"
            << std::endl
            << "We sacrificed a lot of llamas to make this work!"
            << std::endl
            << "Peru is no longer the land of llamas D:"
            << std::endl << std::endl;
  return 0;
}
